...
generated_values["LDR_USERNAME"]="$USER_EMAIL"
...
# --- LOCAL DEEP RESEARCH ---
LDR_PLAIN_PASS="${generated_values["LDR_PASSWORD"]}"
FINAL_LDR_HASH="${generated_values[LDR_PASSWORD_HASH]}"
if [[ -z "$FINAL_LDR_HASH" && -n "$LDR_PLAIN_PASS" ]]; then
    NEW_HASH=$(_generate_and_get_hash "$LDR_PLAIN_PASS")
    if [[ -n "$NEW_HASH" ]]; then
        FINAL_LDR_HASH="$NEW_HASH"
        generated_values["LDR_PASSWORD_HASH"]="$NEW_HASH"
    fi
fi
_update_or_add_env_var "LDR_PASSWORD_HASH" "$FINAL_LDR_HASH"
