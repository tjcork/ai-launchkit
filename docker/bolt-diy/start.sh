...
if [ ! -z "${BOLT_HOSTNAME:-}" ]; then
  ...
  if ! grep -q "allowedHosts:" vite.config.ts; then
    ...
    sed -i "/return {/a\    server: { host: '0.0.0.0', port: 5173, strictPort: false, hmr: { clientPort: 443, protocol: 'wss', host: '${BOLT_HOSTNAME}' }, allowedHosts: ['${BOLT_HOSTNAME}', '.sslip.io', 'localhost'] }," vite.config.ts
