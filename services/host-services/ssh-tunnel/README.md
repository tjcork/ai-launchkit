...
The SSH tunnel token is configured during the initial wizard setup and stored in `host-services/ssh-tunnel/.env`.
...
- `ssh_tunnel_manager.sh` functions: `start_ssh_tunnel()`, `stop_ssh_tunnel()`, `restart_ssh_tunnel()`# Add this section to README.md after the "Quick Start" section
